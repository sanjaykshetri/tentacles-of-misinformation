...
I evaluated four fusion strategies: early fusion (concatenated raw features), late fusion (averaged logits), intermediate fusion (shared representation layer), and stacking (meta-learner on model outputs). The following figure summarizes ROC-AUC, training time, and interpretability trade-offs:
...
Standard k-fold CV is optimistically biased when hyperparameters are tuned on the same folds used for evaluation. I used nested CV (outer 5-fold, inner 3-fold) for unbiased generalization estimates:
